...
class PhotoGallery {
  ...
  async loadImageData() {
    ...
    try {
      ...
      console.log(`总共加载了 ${this.allImages.length} 张图片`);
    } catch (error) {
  ...
  /**
   * 获取所有可能的JSON文件名
   * 按年月格式生成文件名：YYYY-MM.json
   */
  getAllPossibleJsonFiles() {
    const files = [];
  ...
  showImageDetail(image) {
    ...
    <button 
  onclick="this.copyToClipboard('${image.url}')"
  style="margin-left: 8px; padding: 2px 6px; font-size: 12px; cursor: pointer; border: 1px solid #ccc; border-radius: 4px;"
>
  复制
</button>
                </div>
  ...
  copyToClipboard(button, text) {
    navigator.clipboard.writeText(text).then(() => {
      ...
      button.style.backgroundColor = "#48bb78"; // 绿色
